...
*/
	...
	$GLOBALS['PostgreSQLConnectivity_ttr'] = array(
		'bigint(20)'	=> 'bigint',
		'bigint(10)'	=> 'int',
		'int(11)'		=> 'int',
		'tinytext'		=> 'text',
	...
	function PostgreSQLConnectivity_installing( $sql, &$logto)
	{
		global $wpdb;
...
WHERE '.$where : '').';';
		...
		elseif( 0 === strpos( $sql, 'ALTER TABLE'))
		{
			...
			if( 1 === preg_match( $pattern, $sql, $matches))
			{
				...
				if( isset($GLOBALS['PostgreSQLConnectivity_ttr'][$type]))
					$type = $GLOBALS['PostgreSQLConnectivity_ttr'][$type];
				$unsigned = $matches[5];
				$notnull = $matches[6];
				$default = $matches[7];
				$defval = $matches[8];
				if( isset($GLOBALS['PostgreSQLConnectivity_ttr'][$defval]))
					$defval = $GLOBALS['PostgreSQLConnectivity_ttr'][$defval];
			...
			if( 1 === preg_match( $pattern, $sql))
			{
				...
				$sql = str_replace(
					array_keys($GLOBALS['PostgreSQLConnectivity_ttr']), array_values($GLOBALS['PostgreSQLConnectivity_ttr']), $sql);
			}
