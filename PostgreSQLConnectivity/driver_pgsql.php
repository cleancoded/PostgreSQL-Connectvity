<?php
	...
	if ( !extension_loaded('pgsql') )
		wp_die( 'Your PHP installation appears to be missing the PostgreSQL extension which is required by WordPress with PostgreSQLConnectivity.' );

	// Initializing some variables
	$GLOBALS['PostgreSQLConnectivity_version'] = '7.0';
	$GLOBALS['PostgreSQLConnectivity_result'] = 0;
	$GLOBALS['PostgreSQLConnectivity_numrows_query'] = '';
	$GLOBALS['PostgreSQLConnectivity_ins_table'] = '';
	$GLOBALS['PostgreSQLConnectivity_ins_field'] = '';
	$GLOBALS['PostgreSQLConnectivity_last_insert'] = '';
	$GLOBALS['PostgreSQLConnectivity_connstr'] = '';
	$GLOBALS['PostgreSQLConnectivity_conn'] = false;
	...
	{
		if( $GLOBALS['PostgreSQLConnectivity_result'] === false)
			return 0;
		else
			return pg_affected_rows($GLOBALS['PostgreSQLConnectivity_result']);
	}
	...
	function wpsql_error()
		{ if( $GLOBALS['PostgreSQLConnectivity_conn']) return pg_last_error(); else return ''; }
...
****/
	...
	function wpsql_connect($dbserver, $dbuser, $dbpass)
	{
		...
		if( !empty( $hostport[0]))
			$GLOBALS['PostgreSQLConnectivity_connstr'] .= ' host='.$hostport[0];
		if( !empty( $hostport[1]))
			$GLOBALS['PostgreSQLConnectivity_connstr'] .= ' port='.$hostport[1];
		if( !empty( $dbuser))
			$GLOBALS['PostgreSQLConnectivity_connstr'] .= ' user='.$dbuser;
		if( !empty( $dbpass))
			$GLOBALS['PostgreSQLConnectivity_connstr'] .= ' password='.$dbpass;
		elseif( !PostgreSQLConnectivity_INSECURE)
			wp_die( 'Connecting to your PostgreSQL database without a password is considered insecure.
					<br />If you want to do it anyway, please set "PostgreSQLConnectivity_INSECURE" to true in your "db.php" file.' );
		...
		return pg_connect( $GLOBALS['PostgreSQLConnectivity_connstr'].' dbname='.$dbname);
	}
	
	// The effective connection happens here
	function wpsql_select_db($dbname, $connection_id = 0)
	{
		$pg_connstr = $GLOBALS['PostgreSQLConnectivity_connstr'].' dbname='.$dbname;

		// Note:  pg_connect returns existing connection for same connstr
		$GLOBALS['PostgreSQLConnectivity_conn'] = $conn = pg_connect($pg_connstr);
		...
		$ver = pg_version($conn);
		if( isset($ver['server']))
			$GLOBALS['PostgreSQLConnectivity_version'] = $ver['server'];
		...
		if( !empty($GLOBALS['PostgreSQLConnectivity_pre_sql']))
			foreach( $GLOBALS['PostgreSQLConnectivity_pre_sql'] as $sql2run)
				wpsql_query( $sql2run);
		
		PostgreSQLConnectivity_init($conn);
	...
	{
		if( !$GLOBALS['PostgreSQLConnectivity_conn'])
		{
			// Catch SQL to be executed as soon as connected
			$GLOBALS['PostgreSQLConnectivity_pre_sql'][] = $sql;
			return true;
		}
		
		$initial = $sql;
		$sql = PostgreSQLConnectivity_rewrite( $sql);
		
		$GLOBALS['PostgreSQLConnectivity_result'] = pg_query($sql);
		if( (PostgreSQLConnectivity_DEBUG || PostgreSQLConnectivity_LOG_ERRORS) && $GLOBALS['PostgreSQLConnectivity_result'] === false && $err = pg_last_error())
		{
			$ignore = false;
			if( defined('WP_INSTALLING') && WP_INSTALLING)
			{
			...
			if( ! $ignore )
				error_log('['.microtime(true)."] Error running :\n$initial\n---- converted to ----\n$sql\n----> $err\n---------------------\n", 3, PostgreSQLConnectivity_LOG.'PostgreSQLConnectivity_errors.log');
		}
		return $GLOBALS['PostgreSQLConnectivity_result'];
	}
	
	function wpsql_insert_id($lnk = NULL)
	{
		global $wpdb;
		$ins_field = $GLOBALS['PostgreSQLConnectivity_ins_field'];
		$table = $GLOBALS['PostgreSQLConnectivity_ins_table'];
		$lastq = $GLOBALS['PostgreSQLConnectivity_last_insert'];
		...
		elseif('post_author' == $ins_field && false !== strpos($lastq,'ID'))
		{
			...
			$GLOBALS['PostgreSQLConnectivity_queued_query'] = "SELECT SETVAL('$seq',(SELECT MAX(\"ID\") FROM $table)+1);";
		}
		else
		{
			...
			elseif( PostgreSQLConnectivity_DEBUG || PostgreSQLConnectivity_ERROR_LOG)
			{
				$log = '['.microtime(true)."] wpsql_insert_id() was called with '$table' and '$ins_field'".
						" and returned the error:\n".pg_last_error().
						"\nFor the query:\n".$sql.
						"\nThe latest INSERT query was :\n'$lastq'\n";
				error_log( $log, 3, PostgreSQLConnectivity_LOG.'PostgreSQLConnectivity_errors.log');
			}
		}
		if( PostgreSQLConnectivity_DEBUG && $sql)
			error_log( '['.microtime(true)."] Getting inserted ID for '$table' ('$ins_field') : $sql => $data\n", 3, PostgreSQLConnectivity_LOG.'PostgreSQLConnectivity_insertid.log');
	...
	function PostgreSQLConnectivity_rewrite_field($matches)
	{
	...
	function PostgreSQLConnectivity_rewrite( $sql)
	{
		...
		if( 0 === strpos($sql, 'SELECT'))
		{
			...
			if( false !== strpos($sql, 'SQL_CALC_FOUND_ROWS'))
			{
				...
				$GLOBALS['PostgreSQLConnectivity_numrows_query'] = $sql;
				if( PostgreSQLConnectivity_DEBUG)
					error_log( '['.microtime(true)."] Number of rows required for :\n$sql\n---------------------\n", 3, PostgreSQLConnectivity_LOG.'PostgreSQLConnectivity_NUMROWS.log');
			}
			elseif( false !== strpos($sql, 'FOUND_ROWS()'))
			{
				// Here we convert the latest query into a COUNT query
				$sql = $GLOBALS['PostgreSQLConnectivity_numrows_query'];
			...
			$pattern = '/FIELD[ ]*\(([^\),]+),([^\)]+)\)/';
			$sql = preg_replace_callback( $pattern, 'PostgreSQLConnectivity_rewrite_field', $sql);
		...
		{
			require_once( PostgreSQLConnectivity_ROOT.'/driver_pgsql_install.php');
			$sql = PostgreSQLConnectivity_installing( $sql, $logto);
		}
		...
		if( $logto == 'INSERT')
		{
			...
			preg_match($pattern, $sql, $matches);
			$GLOBALS['PostgreSQLConnectivity_ins_table'] = $matches[1];
			$match_list = explode(' ', $matches[0]);
			if( $GLOBALS['PostgreSQLConnectivity_ins_table'])
			{
				$GLOBALS['PostgreSQLConnectivity_ins_field'] = trim($match_list[3],' ()	');
				if(! $GLOBALS['PostgreSQLConnectivity_ins_field'])
					$GLOBALS['PostgreSQLConnectivity_ins_field'] = trim($match_list[4],' ()	');
			}
			$GLOBALS['PostgreSQLConnectivity_last_insert'] = $sql;
		}
		elseif( isset($GLOBALS['PostgreSQLConnectivity_queued_query']))
		{
			pg_query($GLOBALS['PostgreSQLConnectivity_queued_query']);
			unset($GLOBALS['PostgreSQLConnectivity_queued_query']);
		}
		...
		{
			if( $initial != $sql)
				error_log( '['.microtime(true)."] Converting :\n$initial\n---- to ----\n$sql\n---------------------\n", 3, PostgreSQLConnectivity_LOG.'PostgreSQLConnectivity_'.$logto.'.log');
			else
				error_log( '['.microtime(true)."] $sql\n---------------------\n", 3, PostgreSQLConnectivity_LOG.'PostgreSQLConnectivity_unmodified.log');
		}
		return $sql;
	}

	// Database initialization
	function PostgreSQLConnectivity_init()
	{
...
SQL
		...
		if( (PostgreSQLConnectivity_DEBUG || PostgreSQLConnectivity_LOG_ERRORS) && $result === false )
		{
			$err = pg_last_error();
...
/*
	...
	Source : http://vitoriodelage.wordpress.com/2014/06/06/add-missing-wpsql_errno-in-PostgreSQLConnectivity-plugin/
*/
